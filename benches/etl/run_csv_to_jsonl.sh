#!/usr/bin/env bash
# CSV -> JSONL throughput. Seeded dataset: data/bench.csv, 1,000,000 rows,
# seed 42 (id, first_name, country, amount, created_at, active, attributes — a
# mixed-type row; ~116 MiB), generated by gen_bench_data.py. Every tool does a
# one-shot full-file CSV -> local JSONL, one JSON object per input row
# (string-valued fields — matches how mq-bridge's CSV reader, tap-csv and
# sling all emit values). 1 warmup discarded + 5 timed runs, median/stddev,
# output row-count parity check.
#
...
#   Sling:         sling run --src-stream file://… --tgt-object file://…  (optional)
#
...
#   Sling is optional — the block below is skipped if the binary is absent.
set -euo pipefail
...
if [[ ! -x "$MELTANO_BIN" ]]; then
  ...
  for ((i = 1; i <= REPEATS; i++)); do
    ...
    run_meltano_once
    t1="$(now)"
    landed="$(landed_rows "$MELTANO_PROJECT/output/bench.jsonl")"
...
# --- Sling (file CSV -> file JSONL) ---
# Skipped rather than fatal, so the Meltano comparison still runs without it.
SLING_OUT="${SLING_OUT:-/tmp/sling_csv_out.jsonl}"
if [[ ! -x "$SLING_BIN" ]]; then
  ...
  printf 'sling,%s,%s,%s,%s,%s\n' "$ROWS" "$REPEATS" "$sling_median" "$sling_stddev" "$sling_rate" | tee -a "$OUT_CSV"
  rm -rf "$SLING_OUT"
fi
