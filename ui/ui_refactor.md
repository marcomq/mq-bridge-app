...
- No ad-hoc window globals; use localStorage only via the storage layer API.
...
- Messages and history are generated on server but should not be stored there. They should be stored in localStorage, in a different variable as the config. When there is not enough storage anymore in localStorage, remove the oldest history / messages. They can be identified by the id/message_id. It is always in uuidv7 format.
...
- The storage layer API is the approved accessor for localStorage.
...
Hard constraints:
- Do not change the ui appearance or behavior. `npm run test:ui` has a screenshot test. This screenshot test must pass. If a UI appearance or behavior change is intentional, update the snapshot(s), commit them, and rerun `npm run test:ui` to verify the new baseline.
...
Task directive:
Analyze and update this file: append migration map and architecture boundaries, and clean up the document while keeping it aligned with the current refactor direction.
